...
type settings struct {
	...
	SyncInterval        time.Duration `help:"how often this node reconciles its whole membership with one other member, which is what catches anything gossip missed. Lower converges sooner after a fault and costs more traffic, and admits a node sooner: enrolling one waits an interval and a margin, since a sync is what repairs a record a member missed. It is this node's own and need not match its peers" default:"90s"`

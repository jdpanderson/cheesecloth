...
func validCmd() AgentCmd {
	return AgentCmd{settings: settings{Interface: DefaultInterface, OverlayNet: testOverlay, MTU: 1420, WireguardPort: 51820, BindAddr: netip.IPv4Unspecified()}}
}
...
func Test_AgentCmd_Validate_errors(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			AgentCmd{settings: settings{Interface: "../secrets", OverlayNet: testOverlay, MTU: 1420, WireguardPort: 51820}},
			"interface name",
		},
		{
			"overlay too small",
			AgentCmd{settings: settings{Interface: DefaultInterface, OverlayNet: netip.MustParsePrefix("10.0.0.0/31"), MTU: 1420, WireguardPort: 51820}},
			"no room for two nodes",
		},
		{
			"allowed ips inside the overlay",
			AgentCmd{settings: settings{Interface: DefaultInterface, OverlayNet: testOverlay, MTU: 1420, WireguardPort: 51820, AllowedIPs: []netip.Prefix{netip.MustParsePrefix("10.5.0.0/16")}}},
			"overlaps the overlay network",
		},
		{
			"mtu too small",
			AgentCmd{settings: settings{Interface: DefaultInterface, OverlayNet: testOverlay, MTU: 500, WireguardPort: 51820}},
			"unsupported MTU",
		},
		{
			"keepalive not whole seconds",
			AgentCmd{settings: settings{Interface: DefaultInterface, OverlayNet: testOverlay, MTU: 1420, WireguardPort: 51820, PersistentKeepalive: 1500 * time.Millisecond}},
